...
import hmac
import hashlib
class BinanceFuturesClient(Client):
    ...
    def __init__(self, public_key, secret_key, testnet):
        ...
        self.logger.info("Binance Futures Client successfully initialized")
    ...
    def make_request(self, method, endpoint, data):
        ...
        if response.status_code == 200:
            ...
            self.logger.error("Error while making %s request to %s: %s (error code %s)",
    ...
    def get_order_status(self, symbol, order_id):
        ...
        order_status = self.make_request("GET /fapi/v1/allOrders", data)
    ...
    def get_orders(self, symbol):
        ...
        order_status = self.make_request("GET", "/fapi/v2/positionRisk", data)

        return order_status
    def change(self, symbol,margin):

        data = dict()
        data['timestamp'] = int(time.time() * 1000)
        data['symbol'] = symbol
        
        data['marginType']=margin
        data['signature'] = self.generate_signature(data)

        order_status = self.make_request("POST", "/fapi/v1/marginType", data)

        return order_status
    def leveragee(self, symbol,leverage):
        ...
        order_status = self.make_request("POST", "/fapi/v1/leverage", data)
